...
function loadLocale(lang) {
  ...
  try {
    ...
    try {
      return JSON.parse(fs.readFileSync(path.join(__dirname, 'public', 'locales', 'en.json'), 'utf-8'));
    } catch {
      return {};
    }
  }
...
async function qbitLoginWith(url, user, password) {
  ...
  if (res.status === 403) {
    throw new Error(`HTTP 403 from qBittorrent (often a temporary ban after failed logins, or a reverse-proxy blocking the request)`);
  }
  if (!res.ok) {
    throw new Error(`HTTP ${res.status} from qBittorrent${text ? ': ' + text.slice(0, 120) : ''}`);
  }
  if (text === 'Fails.') throw new Error('Invalid username or password');
  if (text !== 'Ok.') throw new Error(`Unexpected response from qBittorrent: ${text.slice(0, 120) || '(empty)'}`);
  const setCookie = res.headers.get('set-cookie');
  return setCookie ? setCookie.split(';')[0] : null;
}

async function qbitLogin() {
  sessionCookie = await qbitLoginWith(config.qbit.url, config.qbit.user, config.qbit.password);
...
async function qbitRequest(endpoint, options = {}) {
  ...
  const doFetch = () => fetch(`${base}${endpoint}`, {
    ...options,
    headers: { 'Cookie': sessionCookie, 'Referer': base, ...(options.headers || {}) }
  });
...
function hasVotedTag(torrent, votedTag) {
  ...
  return torrent.tags.split(',').map(t => t.trim()).includes(votedTag);
}
...
async function checkAndAlert() {
  ...
  try {
    ...
    if (tier > lastAlertedTier) {
      ...
      await sendTelegramMessage(text, config.telegram.botToken, config.telegram.chatId);
...
app.put('/api/language', (req, res) => {
  try {
    const lang = String((req.body || {}).language || '').replace(/[^a-zA-Z_-]/g, '');
...
app.put('/api/settings/qbit', (req, res) => {
  try {
    ...
    const newPassword = typeof qbitIn.password === 'string' ? qbitIn.password : '';
    config.qbit = {
      ...
      votedTag: String(qbitIn.votedTag ?? config.qbit.votedTag).trim() || config.qbit.votedTag
    };
    saveConfig(config);
    sessionCookie = null;
    res.json({ success: true });
...
app.put('/api/settings/telegram', (req, res) => {
  try {
    ...
    config.alerts = {
      threshold: Math.max(1, parseInt(alertsIn.threshold, 10) || ENV_DEFAULTS.alerts.threshold),
      checkIntervalMinutes: Math.max(1, parseInt(alertsIn.checkIntervalMinutes, 10) || ENV_DEFAULTS.alerts.checkIntervalMinutes)
    };
    saveConfig(config);
    restartScheduler();
    res.json({ success: true });
...
app.post('/api/qbit/test', async (req, res) => {
  try {
    ...
    const password = (typeof body.password === 'string' && body.password) ? body.password : config.qbit.password;
    if (!url || !user) return res.status(400).json({ error: 'Missing url or user' });
...
app.post('/api/telegram/test', async (req, res) => {
  try {
    ...
    const botToken = (override.botToken || config.telegram.botToken || '').trim();
    const chatId = (override.chatId || config.telegram.chatId || '').trim();
    if (!botToken || !chatId) {
      return res.status(400).json({ error: 'Missing botToken or chatId' });
    }
    const lang = (override.lang || config.language || 'en').replace(/[^a-zA-Z_-]/g, '');
